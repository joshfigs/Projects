...
public class BigTest {
	...
	throws IOException {
		...
		sc.nextLine();
		String integer = sc.nextLine();
		//Parse the first entered string into a BigInteger
		BigInteger firstBigInteger = BigInteger.parse(integer);
		
		System.out.print("\tEnter second integer => ");
		integer = sc.nextLine();
		//Parse the second entered string into a BigInteger
		BigInteger secondBigInteger = BigInteger.parse(integer);
		//Calculate the sum of the two BigIntegers and print the result
	...
	throws IOException {
		...
		while ((choice = getChoice()) != 'q') {
			switch (choice) {
				case 'p' : parse(); break;	//If user chooses 'p', call the parse() method
				case 'a' : add(); break;	//call add()
				case 'm' : multiply(); break;	//call multiply()
	...
	private static char getChoice() {
		...
		if (in == null || in.length() == 0) {
			choice = ' ';	//Set choice to space
		} else {
			choice = in.toLowerCase().charAt(0);	//Convert user input to lowercase and get the first character
		}
		return choice;
	}
